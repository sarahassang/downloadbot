...
def is_member(user_id):
	...
	get = requests.get(f"https://api.telegram.org/bot{token}/getchatmember?chat_id=@theZhoor_18&user_id={user_id}").text
	if user_id == "@theZhoor_18" or "member" in get or "creator" in get or "administartor" in get:
		two = True
...
(يوتيوب، انستغرام،تيك توك، بنترست،تويتر،سبوتيفاي)
	...
	المالك @Rozs23botللتحميل ارسل الرابط فقط.''',parse_mode="markdown",reply_markup=km([[btn("كيفية التحميل؟",callback_data="help")]]))
